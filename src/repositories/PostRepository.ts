...
export default class PostRepository extends DatabaseRepository {
      ...
      .where((eb) => {
        ...
        if (criteria) {
          ...
          if (criteria.hasAuthor) {
            ...
            filters.push(
              eb.exists(
                this.authorRows(eb.ref("posts.authorId"), criteria.author)
              )
            );
          }
          ...
          if (criteria.hasTags) {
            ...
            filters.push(
              eb.exists(this.tagRows(eb.ref("posts.id"), criteria.tag))
            );
          }
        }
      ...
      .select((eb) => {
        ...
        if (includes) {
          ...
          if (includes.includes("comments")) {
            selections.push(this.comments(eb.ref("posts.id")).as("tags"));
          }
    ...
    const [{ total }] = await this.getDB()
      ...
      .where((eb) => {
        ...
        if (criteria) {
          ...
          //   filters.push(eb.exists(this.commentRows(eb.ref("posts.id"))));
          // } else if (criteria.comments) {
          //   filters.push(
          //     eb.exists(this.commentRows(eb.ref("posts.id"), criteria.comments))
          //   );
          // }
        }
    ...
    return this.getDB()
      ...
      .select((eb) => {
        ...
        if (includes) {
          ...
          if (includes.includes("comments")) {
            selections.push(this.comments(eb.ref("posts.id")).as("comments"));
          }
        }
  ...
  //   postId: Expression<number>,
  //   criteria?: Partial<Comment>
  // ) {
  //   return this.getDB()
  //     .selectFrom("comments as comments")
  //     .whereRef("comments.postId", '=', postId);
  //     .select(sql<number>`1`.as("1"));
  // }
  ...
  protected comments(postId: Expression<number>) {
    return jsonArrayFrom(
      this.getDB()
        .selectFrom("comments")
